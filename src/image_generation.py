...
Prompting strategy:
  Stage 1 — analyze_scene()
    Sends all 4 corner images to gemini-2.0-flash (text-only) and returns a
...
# Nano Banana image-generation model (Gemini 2.0 Flash with image output)
_IMAGE_GEN_MODEL = "gemini-2.0-flash-preview-image-generation"
...
def _build_view_prompt(scene_description: str, view_spec: str) -> str:
    """
...
class ImageGenerator:
    ...
    def analyze_scene(self, images: List[np.ndarray]) -> str:
        ...
        Uses ``gemini-2.0-flash`` (text-only output) so the image-gen quota
        ...
        content_parts: List[types.Part] = [types.Part.from_text(_ANALYSIS_PROMPT)]
    ...
    def generate_viewpoint(
        ...
        prompt = _build_view_prompt(scene_description, view_spec)

        content_parts: List[types.Part] = [types.Part.from_text(prompt)]
    ...
    def generate_additional_views(
        ...
        """
        Generate ``num_views`` new views of the room.

        Args:
            input_images:      The 4 original corner images.
            scene_description: Stage-1 structured room report from analyze_scene().
            num_views:         How many views to generate (max 8, matching the
                               number of pre-defined view specs).
            output_dir:        Directory where generated PNGs are saved.
            output_size:       (width, height) for each output image.
            chain_views:       If True, each generated view is appended to the
                               reference set for the next call (sequential
                               conditioning to reduce cross-view drift).

        Returns:
            List of generated images as uint8 numpy arrays.
        """
        ...
        # Start with just the 4 originals; grow if chain_views is enabled
        ...
        for i, spec in enumerate(specs):
            ...
            try:
                ...
                # Save to disk
                out_path = os.path.join(output_dir, f"generated_view_{i:02d}.png")
                Image.fromarray(img).save(out_path)
                logger.info(f"Saved generated view to: {out_path}")

                # Sequential conditioning: add this view to the reference pool
                if chain_views:
